...
export function HeroScene() {
  ...
  useEffect(() => {
    ...
    core.position.set(7.5, 1.5, 0)
    world.add(core)
    const icoA = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.IcosahedronGeometry(3.4, 1)), new THREE.LineBasicMaterial({ color: 0x6d8bff, transparent: true, opacity: 0.55, blending: THREE.AdditiveBlending, depthWrite: false }))
    const icoB = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.IcosahedronGeometry(2, 0)), new THREE.LineBasicMaterial({ color: 0x46d9ff, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending, depthWrite: false }))
    const coreGlow = new THREE.Sprite(new THREE.SpriteMaterial({ map: glow, color: 0xaebfff, transparent: true, blending: THREE.AdditiveBlending, opacity: 0.85, depthWrite: false }))
    coreGlow.scale.set(9, 9, 1)
    core.add(icoA, icoB, coreGlow)

    const count = window.innerWidth < 720 ? 64 : 116
    const nodes = []
    const seeds = []
    const positions = new Float32Array(count * 3)
    for (let index = 0; index < count; index += 1) {
      const node = new THREE.Vector3((Math.random() - 0.5) * 30 - 3, (Math.random() - 0.5) * 15, (Math.random() - 0.5) * 21)
      nodes.push(node)
      seeds.push({ x: Math.random() * 6.28, y: Math.random() * 6.28, z: Math.random() * 6.28, s: 0.2 + Math.random() * 0.5, base: node.clone() })
    ...
    const points = new THREE.Points(pointGeometry, new THREE.PointsMaterial({ size: 0.58, map: glow, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, color: 0xaebfff, opacity: 0.82 }))
    world.add(points)
    ...
    const lines = new THREE.LineSegments(lineGeometry, new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.28, blending: THREE.AdditiveBlending, depthWrite: false }))
    world.add(lines)
      ...
      for (let i = 0; i < count; i += 1) {
        for (let j = i + 1; j < count; j += 1) {
          ...
          if (distance < 5 && segmentIndex < maxSegments) {
            ...
            position[offset] = a.x; position[offset + 1] = a.y; position[offset + 2] = a.z
            position[offset + 3] = b.x; position[offset + 4] = b.y; position[offset + 5] = b.z
            color[offset] = colorA.r * strength; color[offset + 1] = colorA.g * strength; color[offset + 2] = colorA.b * strength
            color[offset + 3] = colorB.r * strength; color[offset + 4] = colorB.g * strength; color[offset + 5] = colorB.b * strength
            segmentIndex += 1
          }
        }
      }
